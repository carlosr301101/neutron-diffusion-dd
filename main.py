...
from core.inputs import config_dict_fino
...
def code_to_plot():
    ...
    valores_analiticos = np.array(
        [func_analitica(x) for x in x_celdas]
    )  # Flujo analitico para EX1
    ...
    axes[0].set_title(
        f"Comparación: Solución Numérica(Malla: {malla} celdas, Iteraciones: {n_iter})"
    )
